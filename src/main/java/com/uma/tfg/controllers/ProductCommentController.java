...
public class ProductCommentController {
    ...
    public ProductCommentController(ProductService productService, ProductCommentService productCommentService) { this.productService = productService; this.productCommentService = productCommentService;}
    
	@PostMapping("/productComment")
    public void createProductComment(@RequestBody ProductComment request) throws Exception {
        productCommentService.createProductImage(request);
        
        if (request.getProduct() != null) {
    		Product prod = productService.getProduct(request.getProduct().getId());
    		prod.getComments().add(request);
    		
    		productService.updateProduct(prod);
        }
	}
	
	@PutMapping("/productComment")
    public void updateProductComment(@RequestBody ProductComment productComment) throws Exception {
		productCommentService.createProductImage(productComment);
    }
